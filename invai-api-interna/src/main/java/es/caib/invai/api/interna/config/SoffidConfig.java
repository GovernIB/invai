 ...
 * endpoint paths (exposed via generated getters) so callers such as
 * {@link SoffidClient} build request URIs from configuration
...
public class SoffidConfig {
    ...
    /** SCIM 2.0 users search an endpoint path, relative to {@link #baseUrl}. */
    @Getter
    @Value("${es.caib.invai.soffid.user-search-path}")
    private String searchPath;

    /** SCIM 2.0 roles a search endpoint path, relative to {@link #baseUrl}. */
    @Getter
