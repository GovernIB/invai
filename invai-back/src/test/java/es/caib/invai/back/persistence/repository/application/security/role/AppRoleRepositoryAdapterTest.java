package es.caib.invai.back.persistence.repository.application.security.role;

import es.caib.invai.back.persistence.model.application.security.role.AppRoleAudEntity;
import es.caib.invai.back.persistence.model.application.security.role.AppRoleEntity;
import es.caib.invai.back.persistence.model.application.security.core.AppSecurityEntity;
import es.caib.invai.back.persistence.model.maintenance.security.securityRole.SecurityRoleEntity;
import es.caib.invai.back.service.mapper.application.security.role.AppRoleMapper;
import es.caib.invai.back.service.model.application.security.role.AppRole;
import org.junit.jupiter.api.AfterEach;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.mockito.ArgumentCaptor;
import org.mockito.Mock;
import org.mockito.junit.jupiter.MockitoExtension;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageImpl;
import org.springframework.data.domain.Pageable;
import org.springframework.data.jpa.domain.Specification;
import org.springframework.security.core.context.SecurityContextHolder;
import org.springframework.test.util.ReflectionTestUtils;

import java.time.LocalDateTime;
import java.util.List;
import java.util.Optional;

import static org.junit.jupiter.api.Assertions.*;
import static org.mockito.ArgumentMatchers.eq;
import static org.mockito.Mockito.verify;
import static org.mockito.Mockito.when;
import org.mockito.ArgumentMatchers;

/**
 * Unit tests for {@link AppRoleRepositoryAdapter}, verifying entity/model delegation to the
 * {@link AppRoleJPARepository} and the historical audit record written on every mutation.
 */
@ExtendWith(MockitoExtension.class)
class AppRoleRepositoryAdapterTest {

    @Mock
    private AppRoleJPARepository appRoleJPARepository;

    @Mock
    private AppRoleAudJPARepository appRoleAudJPARepository;

    @Mock
    private AppRoleMapper appRoleMapper;

    private AppRoleRepositoryAdapter adapter;

    @AfterEach
    void clearSecurityContext() {
        SecurityContextHolder.clearContext();
    }

    private AppRoleRepositoryAdapter buildAdapter() {
        AppRoleRepositoryAdapter a = new AppRoleRepositoryAdapter();
        ReflectionTestUtils.setField(a, "appRoleJPARepository", appRoleJPARepository);
        ReflectionTestUtils.setField(a, "appRoleAudJPARepository", appRoleAudJPARepository);
        ReflectionTestUtils.setField(a, "appRoleMapper", appRoleMapper);
        return a;
    }

    private AppRoleEntity entityWithRelations(Long id) {
        AppRoleEntity entity = new AppRoleEntity();
        entity.setId(id);
        AppSecurityEntity appSecurity = new AppSecurityEntity();
        appSecurity.setId(10L);
        entity.setAppSecurity(appSecurity);
        SecurityRoleEntity securityRole = new SecurityRoleEntity();
        securityRole.setId(20L);
        entity.setSecurityRole(securityRole);
        return entity;
    }

    @Test
    void findById_found_returnsMappedModel() {
        adapter = buildAdapter();
        AppRoleEntity entity = entityWithRelations(1L);
        AppRole model = new AppRole();
        when(appRoleJPARepository.findById(1L)).thenReturn(Optional.of(entity));
        when(appRoleMapper.toModel(entity)).thenReturn(model);

        AppRole result = adapter.findById(1L);

        assertSame(model, result);
    }

    @Test
    void findById_notFound_returnsNull() {
        adapter = buildAdapter();
        when(appRoleJPARepository.findById(99L)).thenReturn(Optional.empty());

        assertNull(adapter.findById(99L));
    }

    @Test
    void findAll_delegatesToJPARepositoryAndMapsPage() {
        adapter = buildAdapter();
        AppRoleCriteria criteria = new AppRoleCriteria();
        Pageable pageable = Pageable.unpaged();
        AppRoleEntity entity = entityWithRelations(1L);
        AppRole model = new AppRole();
        Page<AppRoleEntity> entityPage = new PageImpl<>(List.of(entity));
        when(appRoleJPARepository.findAll(ArgumentMatchers.<Specification<AppRoleEntity>>any(), eq(pageable))).thenReturn(entityPage);
        when(appRoleMapper.toModel(entity)).thenReturn(model);

        Page<AppRole> result = adapter.findAll(10L, criteria, pageable);

        assertEquals(1, result.getTotalElements());
        assertSame(model, result.getContent().get(0));
    }

    @Test
    void create_savesEntityAndWritesInsertAuditRecord() {
        adapter = buildAdapter();
        AppRole model = new AppRole();
        AppRoleEntity toSave = new AppRoleEntity();
        AppRoleEntity saved = entityWithRelations(5L);
        AppRole response = new AppRole();
        when(appRoleMapper.toEntity(model)).thenReturn(toSave);
        when(appRoleJPARepository.save(toSave)).thenReturn(saved);
        when(appRoleMapper.toModel(saved)).thenReturn(response);

        AppRole result = adapter.create(model);

        assertSame(response, result);
        ArgumentCaptor<AppRoleAudEntity> captor = ArgumentCaptor.forClass(AppRoleAudEntity.class);
        verify(appRoleAudJPARepository).save(captor.capture());
        AppRoleAudEntity aud = captor.getValue();
        assertEquals(5L, aud.getAppRoleId());
        assertEquals(10L, aud.getAppSecurityId());
        assertEquals(20L, aud.getSecurityRoleId());
        assertEquals("INSERT", aud.getAudAction());
        assertNotNull(aud.getCreatedAt());
        assertEquals("SYSTEM_USER", aud.getCreatedBy());
        assertNotNull(aud.getAuditDate());
        assertEquals("SYSTEM_USER", aud.getAuditUser());
    }

    @Test
    void create_entityWithExistingAuditFields_preservesThemOnAuditRecord() {
        adapter = buildAdapter();
        AppRole model = new AppRole();
        AppRoleEntity toSave = new AppRoleEntity();
        AppRoleEntity saved = entityWithRelations(6L);
        LocalDateTime existingCreatedAt = LocalDateTime.of(2025, 1, 1, 0, 0);
        saved.setCreatedAt(existingCreatedAt);
        saved.setCreatedBy("jdoe");
        when(appRoleMapper.toEntity(model)).thenReturn(toSave);
        when(appRoleJPARepository.save(toSave)).thenReturn(saved);

        adapter.create(model);

        ArgumentCaptor<AppRoleAudEntity> captor = ArgumentCaptor.forClass(AppRoleAudEntity.class);
        verify(appRoleAudJPARepository).save(captor.capture());
        AppRoleAudEntity aud = captor.getValue();
        assertEquals(existingCreatedAt, aud.getCreatedAt());
        assertEquals("jdoe", aud.getCreatedBy());
    }

    @Test
    void update_setsIdAndWritesUpdateAuditRecord() {
        adapter = buildAdapter();
        AppRole model = new AppRole();
        AppRoleEntity toSave = new AppRoleEntity();
        AppRoleEntity saved = entityWithRelations(7L);
        AppRole response = new AppRole();
        when(appRoleMapper.toEntity(model)).thenReturn(toSave);
        when(appRoleJPARepository.save(toSave)).thenReturn(saved);
        when(appRoleMapper.toModel(saved)).thenReturn(response);

        AppRole result = adapter.update(model, 7L);

        assertSame(response, result);
        assertEquals(7L, toSave.getId());
        ArgumentCaptor<AppRoleAudEntity> captor = ArgumentCaptor.forClass(AppRoleAudEntity.class);
        verify(appRoleAudJPARepository).save(captor.capture());
        assertEquals("UPDATE", captor.getValue().getAudAction());
    }

    @Test
    void delete_setsIdAndWritesDeleteAuditRecord() {
        adapter = buildAdapter();
        AppRole model = new AppRole();
        model.setId(8L);
        AppRoleEntity toSave = new AppRoleEntity();
        AppRoleEntity saved = entityWithRelations(8L);
        saved.setDeletedAt(LocalDateTime.now());
        saved.setDeletedBy("jdoe");
        when(appRoleMapper.toEntity(model)).thenReturn(toSave);
        when(appRoleJPARepository.save(toSave)).thenReturn(saved);

        adapter.delete(model);

        assertEquals(8L, toSave.getId());
        ArgumentCaptor<AppRoleAudEntity> captor = ArgumentCaptor.forClass(AppRoleAudEntity.class);
        verify(appRoleAudJPARepository).save(captor.capture());
        AppRoleAudEntity aud = captor.getValue();
        assertEquals("DELETE", aud.getAudAction());
        assertNotNull(aud.getDeletedAt());
        assertEquals("jdoe", aud.getDeletedBy());
    }
}
