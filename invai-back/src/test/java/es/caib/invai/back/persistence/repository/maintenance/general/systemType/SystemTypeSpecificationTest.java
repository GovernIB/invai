...
/**
 * Unit tests for {@link SystemTypeSpecification}, verifying that each {@link SystemTypeCriteria} field
 * builds the expected JPA Criteria predicate against a fully mocked {@link CriteriaBuilder}.
 */
@ExtendWith(MockitoExtension.class)
class SystemTypeSpecificationTest {

    @Mock
    private Root<SystemTypeEntity> root;
    ...
    void filterByCriteria_nullCriteria_buildsEmptyConjunction() {
        Specification<SystemTypeEntity> spec = SystemTypeSpecification.filterByCriteria(null);
    ...
    void filterByCriteria_activeStatus_addsIsNullDeletedAtPredicate() {
        ...
        SystemTypeSpecification.filterByCriteria(criteria).toPredicate(root, query, cb);
    ...
    void filterByCriteria_withName_addsLikePredicateOnLoweredName() {
        SystemTypeCriteria criteria = new SystemTypeCriteria();
        criteria.setName("Micro");

        SystemTypeSpecification.filterByCriteria(criteria).toPredicate(root, query, cb);

        verify(cb).like(eq(path), eq("%micro%"));
    }
    ...
    void filterByCriteria_withNameEs_addsLikePredicate() {
        SystemTypeCriteria criteria = new SystemTypeCriteria();
        criteria.setNameEs("Microservicio");

        SystemTypeSpecification.filterByCriteria(criteria).toPredicate(root, query, cb);

        verify(cb).like(eq(path), eq("%microservicio%"));
    }
    ...
    void filterByCriteria_withSearch_orsNameAndNameEsPredicates() {
        SystemTypeCriteria criteria = new SystemTypeCriteria();
        criteria.setSearch("rest");

        SystemTypeSpecification.filterByCriteria(criteria).toPredicate(root, query, cb);

        verify(cb, org.mockito.Mockito.times(2)).like(eq(path), eq("%rest%"));
