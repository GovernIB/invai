 ...
 * to {@link AppSecurityMeasureEntity} records.
 *
 * @since 1.0.4
 */
@Entity
@Table(name = "INV_APP_SECURITY_MEASURE_AUD")
@Getter
@Setter
public class AppSecurityMeasureAudEntity {

    /** Unique identification pointer of this audit trail row. */
    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "INV_APP_SECURITY_MEASURE_AUD_SEQ")
    @SequenceGenerator(name = "INV_APP_SECURITY_MEASURE_AUD_SEQ", sequenceName = "INV_APP_SECURITY_MEASURE_AUD_SEQ", allocationSize = 1)
    @Column(name = "AUDIT_ID", nullable = false, updatable = false)
    private Long auditId;

    /** Identifier of the audited {@code AppSecurityMeasureEntity} record. */
    @Column(name = "APP_SECURITY_MEASURE_ID", nullable = false)
    private Long appSecurityMeasureId;
    ...
    /** Identifier of the associated security measure type at the time of the audited change. */
    @Column(name = "TYPE_ID")
    private Long typeId;

    /** Identifier of the associated ENS requirement at the time of the audited change. */
    @Column(name = "ENS_REQUIREMENT_ID")
