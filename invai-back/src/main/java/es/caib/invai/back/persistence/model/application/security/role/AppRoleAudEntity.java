 ...
 * to {@link AppRoleEntity} records.
 *
 * @since 1.0.4
 */
@Entity
@Table(name = "INV_APP_ROLE_AUD")
@Getter
@Setter
public class AppRoleAudEntity {

    /** Unique identification pointer of this audit trail row. */
    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "INV_APP_ROLE_AUD_SEQ")
    @SequenceGenerator(name = "INV_APP_ROLE_AUD_SEQ", sequenceName = "INV_APP_ROLE_AUD_SEQ", allocationSize = 1)
    @Column(name = "AUDIT_ID", nullable = false, updatable = false)
    private Long auditId;

    /** Identifier of the audited {@code AppRoleEntity} record. */
    ...
    /** Identifier of the security role at the time of the audited change. */
    @Column(name = "SECURITY_ROLE_ID", nullable = false)
