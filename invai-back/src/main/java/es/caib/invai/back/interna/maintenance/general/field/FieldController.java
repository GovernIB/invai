...
public class FieldController {
    ...
    /** Facade service handling the business logic for Field catalog operations. */
     ...
     */
    @GetMapping
    public ResponseEntity<Page<FieldOutputDTO>> getAll(
            @ModelAttribute FieldCriteria filter,
            Pageable pageable) {
        return ResponseEntity.ok(fieldService.getAll(filter, pageable));
    }
    ...
    /**
     * Reactivates a Field and restores its configuration profiles mapping
     * properties corresponding to target database references.
     *
     * @param id the targeted structural identifier element index of the field to reactivate
     * @return a {@link ResponseEntity} containing the current modified configuration state
     *         properties details (as an {@link FieldOutputDTO}) with an HTTP 200 OK status
     */
    @PutMapping("reactivate/{id}")
    public ResponseEntity<FieldOutputDTO> reactivate(@PathVariable Long id) {
        return ResponseEntity.ok(fieldService.reactivate(id));
    }
}
