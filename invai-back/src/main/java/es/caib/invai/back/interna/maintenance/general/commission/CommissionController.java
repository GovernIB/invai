...
public class CommissionController {
    ...
    /** Service facade exposing Commission business use cases to this controller. */
    @Autowired
     ...
     */
    @GetMapping
    public ResponseEntity<Page<CommissionOutputDTO>> getAll(CommissionCriteria criteria, Pageable pageable) {
        return ResponseEntity.ok(commissionService.getAll(criteria, pageable));
    }
    ...
    /**
     * Reactivates a Commission and restores its configuration profiles mapping
     * properties corresponding to target database references.
     *
     * @param id the targeted structural identifier element index of the commission to reactivate
     * @return a {@link ResponseEntity} containing the current modified configuration state
     *         properties details (as an {@link CommissionOutputDTO}) with an HTTP 200 OK status
     */
    @PutMapping("reactivate/{id}")
    public ResponseEntity<CommissionOutputDTO> reactivate(@PathVariable Long id) {
        return ResponseEntity.ok(commissionService.reactivate(id));
    }
}
