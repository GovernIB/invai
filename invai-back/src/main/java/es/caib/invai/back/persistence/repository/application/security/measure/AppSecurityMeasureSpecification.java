...
@Component
@NoArgsConstructor
public class AppSecurityMeasureSpecification {
     ...
     * restricted to security measures belonging to the specified anchor, never to the entire table.
     *
     ...
     */
    public static Specification<AppSecurityMeasureEntity> filterByCriteria(Long appSecurityId, AppSecurityMeasureCriteria criteria) {
        return (root, query, cb) -> {
            ...
            if (criteria.getTypeId() != null) {
                predicates.add(cb.equal(root.get("type").get("id"), criteria.getTypeId()));
            }
            if (criteria.getEnsRequirementId() != null) {
                predicates.add(cb.equal(root.get("ensRequirement").get("id"), criteria.getEnsRequirementId()));
            }
            ...
            if (criteria.getSearch() != null && !criteria.getSearch().trim().isEmpty()) {
                ...
                searchPredicates.add(cb.like(cb.lower(root.get("type").get("name")), searchPattern));
                searchPredicates.add(cb.like(cb.lower(root.get("ensRequirement").get("name")), searchPattern));
