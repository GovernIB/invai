...
@Component({
  ...
  standalone: true,
  imports: [CrudEntityDialog, DatePicker, FloatLabel, InputText, ReactiveFormsModule, Select],
...
export class ApplicationProviderDialog {
  ...
  protected readonly title = computed(() => APPLICATION_PROVIDER_DIALOG_TITLES[this.mode()]);
  ...
  protected readonly maxLengthError = APPLICATION_PROVIDER_DIALOG_MAX_LENGTH_ERROR;
  protected readonly dateRangeError = APPLICATION_PROVIDER_DIALOG_DATE_RANGE_ERROR;
  protected readonly dateFormat = APPLICATION_PROVIDER_DIALOG_DATE_FORMAT;
  protected readonly datePlaceholder = APPLICATION_PROVIDER_DIALOG_DATE_PLACEHOLDER;
  protected readonly actionAriaLabels = APPLICATION_PROVIDER_DIALOG_ARIA_LABELS;
  ...
  protected datePassThrough(): DatePickerPassThrough {
    ...
    return {
      pcInputText: {
        root: {
          'aria-describedby': invalid ? 'application-provider-dialog-date-range-error' : null,
  ...
  protected rolePassThrough(): SelectPassThrough {
    const invalid = this.isInvalid(this.form().controls.roleId);
    return {
      label: {
        'aria-describedby': invalid ? 'application-provider-dialog-role-error' : null,
        'aria-invalid': String(invalid),
      },
    };
  }
