...
describe('accessibility resource HTTP services', () => {
  ...
  it('separates active, inactive and unfiltered pages and retries failed entries', () => {
    ...
    const base = '/invaiback/classification-segment';
  ...
  it.each(['create', 'update', 'deactivate', 'reactivate'] as const)(
    ...
    (operation) => {
      ...
      const base = '/invaiback/compliance-situation';
  ...
  it.each([
    ...
    http.expectOne(`/invaiback/${endpoint}?page=0&size=10`).flush(page([]));
  });
  ...
  it('sends every bilingual criterion and reuses a cached page', () => {
    ...
    http
      .expectOne(
        '/invaiback/classification-segment?page=1&size=25&sort=name,desc&name=Acc%C3%A9s&nameEs=Acceso&statusId=2',
      )
      .flush(page([]));
  });
  ...
  it('does not cache quick search and invalidates pages after a successful mutation', () => {
    ...
    const searches = http.match(
      (request) =>
        request.url === '/invaiback/compliance-situation' &&
    ...
    service.getAll({ page: 0, size: 10 }).subscribe();
    http.expectOne('/invaiback/compliance-situation?page=0&size=10').flush(page([]));
    service.create({ name: 'Públic', nameEs: 'Público' }).subscribe();
    const create = http.expectOne('/invaiback/compliance-situation');
